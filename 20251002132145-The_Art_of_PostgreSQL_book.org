...
* 3. Getting Ready to read this Book
  ...
  #+begin_src sh
  $ docker exec -it the_art_of_postgresql_book psql -U postgres -c "create database chinook"
  $ curl -fsSL "https://raw.githubusercontent.com/xivSolutions/ChinookDb_Pg_Modified/ed51a94152c31ba7b81fdc0bc7e32edca056a86a/chinook_pg_serial_pk_proper_naming.sql" | docker exec -i the_art_of_postgresql_book psql -U postgres -d chinook
  #+end_src
* 4. Business Logic
...
** Every SQL query embeds some business logic
   ...
   - The /from/ clause only mentions the /track/ trable, somehow we decided that's
     ...
     implemented
   - The /where/ clause restricts the data output to the /album_id/ 193 which again
   ...
   - Finally, the /order by/ clause implements the idead that we want to display
     ...
     Not only that, it also incorporates the specific knowledge that the /track_id/
...
** Business Logic Applies to Use Cases
   ...
   DEBUGSQL = False
   PGCONNSTRING = "host=localhost user=postgres dbname=chinook password=1234"
   ...
   class Track(Model):
       tablename = "track"
       columns = ["track_id", "name", "milliseconds", "bytes", "unit_price"]
   ...
   if __name__ == "__main__":
       if len(sys.argv) > 1:
           ...
           for album in Album.fetchall(pgconn, artist_id=artist.id):
               ms = 0
               for track in Track.fetchall(pgconn, album_id=album.id):
                   ms += track.duration
...
** Efficiency
   ...
   - Then we do another network round-trip to fetch a list of albums for the artist_id
